...
**Current stage: Milestone 0 — repository foundation only.**

No cashier workflow, products, stock, sales, payments, receipts, M-Pesa, or
eTIMS behaviour exists yet. Do not treat this README as a feature list.
...
Cashier browser (kiosk)
        ↓
Application / API  (FastAPI — not implemented in M0)
        ↓
Domain
        ↓
Persistence
...
## Development status (Milestone 0)

| Area | Status |
|---|---|
| Repository, docs, Python package, environment test | IMPLEMENTED / TESTED (M0) |
| Login, products, inventory, cash sales, receipts | PLANNED |
| Weighted goods, barcode, printer, scale | PLANNED |
...
Milestone 0 only checks that the `duka_pos` package imports. The first business
acceptance test (weighted rice sale) belongs to Milestone 1 and has not been
written.
