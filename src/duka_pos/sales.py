...
MPESA path (M3.1 local foundation; M3.2 STK after commit):
...
Idempotency: client_reference is UNIQUE on sales.
...
def create_sale(
    ...
    *,
    ...
    if payment_method == "MPESA":
        phone_number = normalize_ke_msisdn(phone_number)
    ...
    provider = cash_provider.provider_name if immediate else mpesa_daraja_provider.provider_name
    now = _now_iso()
    ...
    try:
        with db_module.transaction(conn):
            ...
            for product_id, quantity_milli in parsed_lines:
                ...
                line_results.append((product, quantity_milli, product.selling_price_cents, total_for_line))
                if immediate:
                    conn.execute(
                        "UPDATE stock_balances SET quantity_milli = quantity_milli - ? WHERE product_id = ?",
            ...
            try:
                cursor = conn.execute(
                    ...
                    (client_reference, sale_status, subtotal_cents, total_cents, now, user_id, shift_id),
                )
            ...
            for product, quantity_milli, unit_price_cents, line_total in line_results:
                conn.execute(
                    ...
                    (sale_id, product.id, quantity_milli, unit_price_cents, product.cost_price_cents, line_total),
                )
                if immediate:
                    conn.execute(
                        ...
                        (product.id, -quantity_milli, product.cost_price_cents, sale_id, now, user_id),
                    )
            conn.execute(
                ...
                (sale_id, payment_method, payment_status, total_cents, now, provider, phone_number, client_payment_reference, now),
            )
            if immediate:
                receipt_number = f"RCPT-{sale_id:08d}"
                conn.execute(
                    "INSERT INTO receipts (sale_id, receipt_number, created_at) VALUES (?, ?, ?)",
...
def get_sale(conn: sqlite3.Connection, sale_id: int) -> Sale:
    ...
    line_rows = conn.execute(
        """SELECT sl.*, p.name AS product_name FROM sale_lines sl
        JOIN products p ON p.id = sl.product_id WHERE sl.sale_id = ? ORDER BY sl.id""",
        (sale_id,),
    ).fetchall()
    lines = [
        SaleLineResult(
            product_id=row["product_id"], product_name=row["product_name"],
            quantity_milli=row["quantity_milli"], unit_price_cents=row["unit_price_cents"],
            unit_cost_cents=row["unit_cost_cents"], line_total_cents=row["line_total_cents"],
        )
        for row in line_rows
    ]
    payment_row = conn.execute(
        "SELECT * FROM payments WHERE sale_id = ? ORDER BY id DESC LIMIT 1", (sale_id,)
    ).fetchone()
    payment = None
    if payment_row is not None:
        keys = payment_row.keys()
        payment = PaymentResult(
            id=payment_row["id"], method=payment_row["method"], status=payment_row["status"],
            amount_cents=payment_row["amount_cents"], created_at=payment_row["created_at"],
            provider=payment_row["provider"] if "provider" in keys else None,
            provider_checkout_request_id=payment_row["provider_checkout_request_id"] if "provider_checkout_request_id" in keys else None,
            provider_merchant_request_id=payment_row["provider_merchant_request_id"] if "provider_merchant_request_id" in keys else None,
            provider_receipt_number=payment_row["provider_receipt_number"] if "provider_receipt_number" in keys else None,
            phone_number=payment_row["phone_number"] if "phone_number" in keys else None,
            client_payment_reference=payment_row["client_payment_reference"] if "client_payment_reference" in keys else None,
        )
    return Sale(
        id=sale_row["id"], client_reference=sale_row["client_reference"], status=sale_row["status"],
        subtotal_cents=sale_row["subtotal_cents"], total_cents=sale_row["total_cents"],
        created_at=sale_row["created_at"], lines=lines, payment=payment,
    )


def get_receipt(conn: sqlite3.Connection, sale_id: int) -> Receipt:
    receipt_row = conn.execute("SELECT * FROM receipts WHERE sale_id = ?", (sale_id,)).fetchone()
    if receipt_row is None:
        raise ReceiptNotFound(f"receipt for sale {sale_id} not found")
    sale = get_sale(conn, sale_id)
    lines = [
        ReceiptLine(
            product_name=line.product_name, quantity_milli=line.quantity_milli,
    ...
    return Receipt(
        sale_id=sale.id, receipt_number=receipt_row["receipt_number"], created_at=receipt_row["created_at"],
        lines=lines, subtotal_cents=sale.subtotal_cents, total_cents=sale.total_cents,
